...
def generate_text(occupation, condition, experience, weakness, business_content, purpose, lower_limit, upper_limit):
    ...
    if matched_experience == 1:
        ...
        prompt = "###指示###新卒の就活生の立場で、サマーインターンの志望動機を考えてください。企業理念はそのまま使わないでください。自身の弱みはそのまま使わずポジティブにして強みとして使ってください。会社名はそのまま使わず貴社に変換してください。\n###応募職種###：{occupation}\n###自身の弱み###{weakness}\n###会社の事業内容###{business_content}\n###企業理念###{purpose}\n###文字数の下限###{lower_limit}\n###文字数の上限###{upper_limit}".format(occupation=occupation, matched_experience=experience, weakness=weakness, business_content=business_content, purpose=purpose, lower_limit=lower_limit, upper_limit=upper_limit)
